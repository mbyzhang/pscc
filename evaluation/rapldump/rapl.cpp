...
static std::vector<float> gTotals_W;
...
static void Finish() {
  ...
  for (float& iter : gTotals_W) {
    double deviation = (iter - mean);
    sumOfSquaredDeviations += deviation * deviation;
  }
  double popStdDev = sqrt(sumOfSquaredDeviations / n);

  printf("mean = %5.2f W, stdev = %5.2f W\n", mean, popStdDev);

  std::ofstream out("record.raw", std::ios::out | std::ios::binary);
  out.write(reinterpret_cast<const char*>(gTotals_W.data()), gTotals_W.size() * sizeof(float));
  out.close();
...
static void SigAlrmHandler(int aSigNum, siginfo_t* aInfo, void* aContext) {
  ...
  gTotals_W.push_back(JoulesToWatts(cores_J));
...
static void PrintUsage() {
  printf(
      ...
      "  -f --sample-frequency <N> sample frequency in hertz [default=1]\n"
...
int main(int argc, char** argv) {
  ...
  while ((c = getopt_long(argc, argv, shortOptions, longOptions, NULL)) != -1) {
    switch (c) {
      ...
      case 'f':
        sampleFrequency = strtod(optarg, &endPtr);
        if (*endPtr) {
          CmdLineAbort("sample frequency is not a float");
        }
        break;
  ...
  // }
  gSampleInterval_sec = 1.0 / sampleFrequency;
  ...
  double whole;
  double frac = modf(gSampleInterval_sec, &whole);
  timer.it_interval.tv_sec = (time_t)whole;
  timer.it_interval.tv_usec = (suseconds_t)(frac * 1e6);
